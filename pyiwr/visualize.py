...
def marginal_max(xg, radar_location='SOHRA', field_name='DBZ', show_rings=False, show_grid=False, show_cross_sections=True, save_image=False, img_name=None):
    ...
    if show_cross_sections:
        ...
        # Plot cross-sections 
        xg[field_name][0].max(axis=1).plot.contourf(cmap=cmap, levels=range(cmap_levels[0], cmap_levels[1]+1), add_colorbar=False, ax=ax_x)
        xg[field_name][0].max(axis=2).T.plot.contourf(cmap=cmap, levels=range(cmap_levels[0], cmap_levels[1]+1), add_colorbar=False, ax=ax_y)
